...
var HttpError = require('./err/http_error').HttpError;
...
function Route(options) {
  ...
  this.__defineSetter__('url', function(u) {
    ...
    self.pattern = '^';
    self.params = [];
    self._url.split('/').forEach(function(u) {
      if (!u.length)
        return;
...
Route.prototype.matchesVersion = function matchesVersion(req) {
  ...
  if (this.version) {
    ...
    if (!semver.satisfies(this.version, req.headers['x-api-version']))
      return false;
  }
...
Route.prototype.run = function run(req, res) {
  ...
  function _run(err) {
    if (err) {
      ...
      if (chain[i])
        return chain[i++].call(self, req, res, _run);
    }
