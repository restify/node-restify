...
function Route(options) {
  ...
  if (typeof(options.Logger) !== 'object')
    throw new TypeError('options.Logger (Object) required');
  ...
  if (options.version) {
    if(options.rangeVersioning && !semver.validRange(options.version)){
        throw new Error('only semver range versions are allowed');
    } else if(!semver.valid(options.version) && !options.rangeVersioning) {
        throw new Error('only semver versions are allowed');
    }
  }
  ...
  this.log = this.Logger.child({component: 'restify/route', name: self.name });
