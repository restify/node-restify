...
Response.prototype.send = function send(code, body, headers) {
    ...
    function _cb(err, _body) {
        var
          noneMatch = self.req.headers['if-none-match'],
          etagMatches = false,
          etag;
        ...
        if(self.etag === true) {
          ...
          // Set ETag: GET and wasn't set manually (HEAD doesn't have body)
          if(self.req.method === 'GET' && !self.getHeaders()['etag']) {
            if(_body && body.length) {
              etag = utils.etag(_body);
              self.setHeader('ETag', etag);
            }
          }

          // Should send 304 (2xx, GET or HEAD)
          if (self.statusCode >= 200 && self.statusCode < 300 && noneMatch && (self.req.method === 'GET' || self.req.method === 'HEAD')) {
            etag = etag || utils.etag(_body);

            // https://github.com/visionmedia/node-fresh/blob/master/index.js#L43
            etagMatches = ~noneMatch.indexOf(etag) || '*' == noneMatch[0];

            self.statusCode = etagMatches ? 304 : self.statusCode;
          }
        }
