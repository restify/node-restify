...
function formatText(req, res, body) {
  ...
  if (body instanceof Error) {
    res.code = (body instanceof errors.HttpError) ? body.code : 500;
...
function formatJSON(req, res, body) {
  ...
  if (body instanceof Error) {
    var _body = body;
    if (body instanceof errors.RestError) {
      res.code = _body.code;
      body = _body.body;
    } else {
      res.code = (_body instanceof errors.HttpError) ? _body.code : 500;
      body = {
        message: _body.message
      };
    }

    if (process.env.NODE_ENV === 'development' && _body.stack)
      body.stack = _body.stack;
  }
...
function Response(options) {
  ...
  if (options.formatters) {
    Object.keys(options.formatters).forEach(function(k) {
      var type = mime.lookup(k);
      self.formatters[type] = options.formatters[k];
    });
  }
...
Response.prototype.format = function format(body) {
  ...
  if (!~this.types.indexOf(this.contentType)) {
    if (Buffer.isBuffer(body)) {
      this.contentType = 'application/octet-stream';
    } else {
      this.contentType = 'text/plain';
  ...
  var formatter = this.formatters[this.contentType];
