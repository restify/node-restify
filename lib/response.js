    ...
    function redirect(arg1, arg2, arg3) {
        ...
        if (arg1 && !isNaN(arg1)) {
            ...
            if (opt.query) {
                ...
                Object.keys(opt.query).forEach(function forEach(k) {
                    var v = opt.query[k];
                    var values = Array.isArray(v) ? v : [v];
                    values.forEach(function forEachItem(item) {
                        merged.append(k, item);
                    });
                });

                if (opt.overrideQuery !== true) {
                    finalUri.searchParams.forEach(function forEach(v, k) {
                        merged.append(k, v);
                    });
                }

                finalUri.search = merged.toString();
            }
