...
Response.prototype.send = function send(code, body, headers) {
    ...
    function _cb(err, _body) {
        ...
        // Etag: enabled and wasn't set manually
        if(self.etag === true && !self.getHeaders()['etag']) {
          etag = utils.etag(_body);
          self.setHeader('ETag', etag);

          // non modified
          // https://github.com/visionmedia/express/blob/cec0c06a70c874298761282984573475293c0071/lib/request.js#L405
          if(self.statusCode >= 200 && self.statusCode < 300 && self.req.headers['if-none-match'] === etag) {
            self.statusCode = 304;
          }
        }
