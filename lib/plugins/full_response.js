...
function setHeaders(req, res) {
        ...
        if (res.version && !req.getHeader('X-Api-Version'))
                res.setHeader('X-Api-Version', res.version);

        if (!res.getHeader('X-Request-Id'))
                res.setHeader('X-Request-Id', req.getId());

        if (!res.getHeader('X-Response-Time'))
                res.setHeader('X-Response-Time', now.getTime() - req._time);
