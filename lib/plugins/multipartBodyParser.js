                ...
                if (opts.mapFiles) {
                    ...
                    Object.keys(files).forEach(function forEach(f) {
                        ...
                        if (Array.isArray(files[f])) {
                            req.params[f] = [];
                            files[f].forEach(function forEach2(file) {
                                barrier.start('fs' + file.name);
                                fs.readFile(file.path, function readFile(
                                    ex,
                                    data
                                ) {
                                    barrier.done('fs' + file.name);
                                    if (ex) {
                                        return next(
