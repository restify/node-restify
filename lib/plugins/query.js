 ...
 * @param {Number} [options.arrayLimit=20] Only transform `?a[$index]=b` to an 
 * array if `$index` is less than `arrayLimit`.
 * @param {Number} [options.depth=5] The depth limit for parsing nested objects,
 * e.g. `?a[b][c][d][e][f][g][h][i]=j`.
 * @param {Number} [options.parameterLimit=1000] Maximum number of query params 
 * parsed. Additional params are silently dropped.
 * @param {Boolean} [options.parseArrays=true] Whether to parse `?a[]=b&a[1]=c` 
 ...
 * @param {Boolean} [options.strictNullHandling=false] If true, `?a&b=` results 
 * in `{a: null, b: ''}`. Otherwise, `{a: '', b: ''}`.
 * @returns  {Function}
 * @example
