...
function serveStatic(opts) {
        ...
        function serve(req, res, next) {
								...
								if(opts.directory) {
									var route = req.route.path.toString();
									route = route.substring(1,route.length-4).replace(/\\\//g,'/');

									if(opts.directory.slice(-1) != '/') { opts.directory += '/' }
                ...
                if (!re.test(file.replace(/\\/g, '/'))) {
									if(opts.default && file.indexOf(opts.default) < 0) {
										if(file.slice(-1) == '/') {
