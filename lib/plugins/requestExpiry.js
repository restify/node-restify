...
function requestExpiry(options) {
    ...
    return function requestExpirationCheck(req, res, next) {
        /*
         * Add check expiry API to to req if it doesn't already exist. Check
         * because this function could be used in multiple places in the
         * handler chain.
         */
        if (!req._expiry) {
            // if the headers don't exist, then the request will never expire.
            req._expiry = {
                _expiryTime: Infinity,
                isExpired: function isExpired() {
                    return Date.now() > req.expiry._expiryTime;
                }
            };
            if (useAbsolute) {
                req._expiry._expiryTime = Number(req.headers[absoluteHeaderKey]);
            } else {
                ...
                if (startTime && timeout) {
                    req._expiry._expiryTime = Number(startTime) + Number(timeout);
                }
            }

            req.isExpired = req._expiry.isExpired;
        }
