  ...
  return function rateLimit(req, res, next) {
    ...
    if (!attr) {
      if (req.log.isTraceEnabled())
        req.log.trace('Throttle: no request attribute found (done)', options);
      return next();
    }
    ...
    if (!hash[attr].consume(1)) {
      ...
      // Until https://github.com/joyent/node/pull/237 is in
      return next(new HttpError(429, sprintf(MESSAGE, rate)));
    }
