 ...
 */
function pngBodyParser(options) {
  if (options && typeof (options) !== 'object')
    throw new TypeError('options (Object) required');
  if (!options)
    options = {};

  return function parsePngBody(req, res, next) {
    ...
    req.on('end', function () {
      ...
      if (req.body.toString('hex', 0, 4) !== '89504e47')
        return next(new InvalidContentError('Invalid PNG file'));

      try {
        if (options.mapParams !== false) {
          req.params.png = req.body;
        } else {
          req._body = req.body;
          req.body = req.params;
        }
