 ...
 * If-Match and If-None-Match header, and an already set Last-Modified header with 
 * the client's If-Modified-Since and If-Unmodified-Since header.
 *
...
function conditionalRequest() {
  return function checkConditions(req, res, next) {
    ...
    if (modified && typeof(modified) === 'string' && isValidDate(modified)) {
      ...
      if (req.headers['if-modified-since'] && isValidDate(req.headers['if-modified-since'])) {
      ...
      if (req.headers['if-unmodified-since'] && isValidDate(req.headers['if-unmodified-since'])) {
