...
Server.prototype.inflightData = function inflightData() {
    var self = this;
...
Server.prototype._run = function _run(req, res, route, chain, cb) {
    ...
    function next(arg) {
        ...
        if (!done && chain[++i] && !_reqClosed(req)) {
            ...
            // determime if we are in pre or use. pre is before routing, so
            // route should not exist.
            var inflightStore;

            if (req.route) {
                if (!self._inflight.routes.hasOwnProperty(req.route.path)) {
                    self._inflight.routes[req.route.path] = [];
                }
                if (!self._inflight.routes[req.route.path][i]) {
                    self._inflight.routes[req.route.path][i] = {
                        name: handlerName,
                        count: 0
                    };
                }
                inflightStore = self._inflight.routes[req.route.path];
            } else {
                if (!self._inflight.pre[i]) {
                    self._inflight.pre[i] = {
                        name: handlerName,
                        count: 0
                    };
                }
                inflightStore = self._inflight.pre;
            }
            ...
            if (!inflightStore[i]) {
                ...
                inflightStore[i].count++;
                self._inflight.requests++;
            }

            var nWrapper = function nWrapper(a) {
                inflightStore[i].count--;
                self._inflight.requests--;
                once(next)(a);
            };
            ...
            if (self.handleUncaughtExceptions === true) {
                nWrapper.ifError = ifError(nWrapper);
            }

            return (chain[i].call(self, req, res, nWrapper));
        }
