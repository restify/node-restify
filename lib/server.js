...
var assertBoolean = args.assertBoolean;
...
function argsToChain() {
        ...
        function process(handlers) {
                for (var i = 0; i < handlers.length; i++) {
                        if (Array.isArray(handlers[i]))
                                return (process(handlers[i]));

                        assertFunction('handler', handlers[i]);
        ...
        return (process(Array.prototype.slice.call(_args, 0)));
}
...
function Server(options) {
        ...
        assertObject('options.dtrace', options.dtrace);
        assertObject('options.log', options.log);
        assertObject('options.router', options.router);
        ...
        if (options.formatters) {
                Object.keys(options.formatters).forEach(function (k) {
                        assertFunction(options.formatters[k]);
...
Server.prototype.listen = function listen(port, host, callback) {
        ...
        case 'function':
                callback = port;
                return (this.server.listen(_callback));
        ...
        case 'string':
                ...
                throw new TypeError(port + ' is not a named pipe');
...
['del', 'get', 'head', 'post', 'put', 'patch'].forEach(function (method) {
        ...
        var functor;
        var tmp = function REPLACE_ME(opts) {
                var self = this;
                ...
                assertFunction('handler', arguments[1]);

                if (method === 'del')
                        method = 'DELETE';

                opts.method = method.toUpperCase();
                var route = this.router.mount(opts);
                if (!route)
                        return (null);
                ...
                var chain = [];
                var i = 0;
                var name = opts.name || (method + '-' + url).replace(/\W/g, '');
                name = name.toLowerCase();
                var probes = dtrace.addProbes(this.dtp, name);

                function addHandler(h) {
                        chain.push(h);

                        var _name = h.name || '' + (i + 1);
                        _name = name + '-' + _name;
                        var _probes = dtrace.addProbes(self.dtp, _name);
                ...
                argsToChain(_args).forEach(addHandler);
                this.routes[route] = chain;

                return (route);
        }.toString().replace(/^function REPLACE_ME/, 'function ' + method);

        // This little bit of fugly is so the function name is set and shows
        // up in stack traces
        assert.ok(eval('functor = ' + tmp));
        Server.prototype[method] = functor;
});
...
Server.prototype.use = function use() {
        ...
        (argsToChain(arguments) || []).forEach(function (h) {
                self.chain.push(h);
        });
...
Server.prototype._handle = function _handle(req, res) {
        ...
        function _route() {
                if (log.trace())
                        log.trace({req: req}, 'checking for route');
                ...
                self.router.find(req, res, function (err, r, ctx) {
                        if (err) {
                                log.trace({
                                        err: err,
                                        req: req
                                }, 'router errored out; responding to client');
                                return (res.send(err));
                        }
                        ...
                        if (log.trace())
                                log.trace({req: req}, 'route found');

                        req.context = ctx;
                        return (self._run(req, res, r, self.routes[r]));
                });
                return (false);
        }
        ...
        function next(err) {
                ...
                if (!done && chain[++i]) {
                        ...
                        try {
                                ...
                                log.warn({err: e}, 'uncaughtException');
