...
log.trace(options.contentHandlers)
        ...
        if (options.contentHandlers.hasOwnProperty(k)) {
          if (typeof(options.contentHandlers[k]) === 'function')
            ...
            try {
              ...
              server._config.contentHandlers[k] = require('./contentTypes/'+type[2]).contentHandler();
            } catch (e) {
              throw new Error('could not import contentHandler application/'+type[2]);
            }
          } else 
        ...
        if (options.contentWriters.hasOwnProperty(k)) {
          if (typeof(options.contentWriters[k]) === 'function')
            ...
            try {
              ...
              server._config.contentWriters[k] = require('./contentTypes/'+type[2]).contentWriter();
            } catch (e) {
