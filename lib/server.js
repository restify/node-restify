...
/**
 * Add a new HTTP verb to the server which can later be access using
 * `server.[verb](` like the built-in verbs. If the verb already exists or is
 * a name collision with an existing property on the object, this function is
 * a no-op.
 *
 * @public
 * @function addVerb
 * @param {String} verb - The verb to add to the server
 * @returns {null}
 */
Server.prototype.addVerb = function addVerb(verb) {
    if (typeof verb !== 'string') {
        throw new TypeError('verb name expected to be string');
    }
    verb = verb.toLowerCase();
    // Check to see if the route is already defined, if so short-circuit.
    ...
    this[verb] = function (opts) {
        ...
        if (!opts.name) {
            opts.name = verb + '-' + (opts.path || opts.url);
...
// Register all the default routing methods
/**
...
[
    ...
    'put',
    'patch'
].forEach(Server.prototype.addVerb, Server.prototype);
