...
/**
 * Wrap a service into a Proxy Protocol connection.  This will look at the
 * proxyProtocol setting and determine if it should use the unmodified
 * transport or if it should wrap the transport with proxywrap.
 * @private
 * @function proxyProtocolWrap
 * @param    {Object}            transport to conditionally wrap
 * @param    {Boolean | Object}  options or enabling flag
 * @returns  {Object}
 */
function wrapProxyProtocol(transport, options) {
    if (!options) {
        return transport;
    }

    if (transport === spdy) {
        // SPDY module needs to be wrapped slightly differently
        transport = spdy.server;
    }

    if (typeof options === 'object') {
        return proxyWrap.proxy(transport, options);
    }

    return proxyWrap.proxy(transport);
}
...
function Server(options) {
    ...
    if (options.spdy) {
        this.spdy = true;
        transport = wrapProxyProtocol(spdy, options.proxyProtocol);
        this.server = transport.createServer(options.spdy);
        ...
        this.secure = true;
        transport = wrapProxyProtocol(https, options.proxyProtocol);
        this.server = transport.createServer({
            ca: self.ca,
        ...
        transport = wrapProxyProtocol(https, options.proxyProtocol);
        this.server = transport.createServer(options.httpsServerOptions);
    } else {
        transport = wrapProxyProtocol(http, options.proxyProtocol);
        this.server = transport.createServer();
    }
