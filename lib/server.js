...
Server.prototype._run = function _run(req, res, route, chain, cb) {
    ...
    d.on('error', function onError(err) {
        if (err._restify_next) {
            ...
            var uncaughtEx = new errors.UncaughtExceptionError(
                err, 'uncaught exception in domain!'
            );
            self._finishReqResCycle(req, res, route, uncaughtEx);
        }
    });
    d.run(n1);
};
...
function _finishReqResCycle(req, res, route, err) {
    ...
    if (!res.finished && !req._connectionState) {
