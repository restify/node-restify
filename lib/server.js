...
function Server(options) {
  ...
  this.log = this.log4js.getLogger('RESTServer');
  this.routes = [];
  ...
  if (!options.noParsers) {
    this.use(parsers.getAcceptParser('json'));
...
Server.prototype._request = function _request(req, res) {
  ...
  var route = this._findRoute(req, res);

  return route ? route.run(request, res) : false;
};
...
Server.prototype._findRoute = function _findRoute(req, res) {
  ...
  for (var i = 0; i < this.routes.length; i++) {
    route = this.routes[i];
    var matched = false;
    params = route.matchesUrl(req);
    if (params) {
      notFound = false;
      matched = true;
    }
    if (route.matchesMethod(req)) {
      notMethod = false;
      matched &= true;
    }
    if (route.matchesVersion(req)) {
      notVersion = false;
      matched &= true;
    }
    if (matched)
