...
var REGEXP_PATH_DEPRECATED_TEXT =
    'RegExp path is deprecated, see: http://restify.com/docs/6to7';
...
function serverMethodFactory(method) {
    /**
     * @private
     * @param {String|Object} opts - path string or config object
     * @param {String} [opts.path] - path
     * @param {String} [opts.url] - alias for path
     * @returns {String} name of the route
     * @example
     * server.get('/example/near/:lat-:lng/radius/:r', (req, res, next) => {});
     * server.get({ path: '/foo' }, (req, res, next) => {});
     * server.get({ url: '/foo' }, (req, res, next) => {});
     */
    return function serverMethod(opts) {
        // RegExp deprecation message
        if (opts instanceof RegExp) {
            throw new TypeError(REGEXP_PATH_DEPRECATED_TEXT);
        }
        ...
        // RegExp deprecation message
        if (opts.path instanceof RegExp) {
            throw new TypeError(REGEXP_PATH_DEPRECATED_TEXT);
        }
