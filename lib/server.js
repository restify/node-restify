...
module.exports = {
          ...
          if (options.contentHandlers.hasOwnProperty(k)) {
            if (typeof(options.contentHandlers[k]) === 'function')
              ...
              try {
                ...
                server._config.contentHandlers[k] = require('./contentWriters/'+type[1]);
              } catch (e) {
                throw new Error('could not import contentWriter '+options.contentHandlers[k])
              }
            } else 
          ...
          if (options.contentWriters.hasOwnProperty(k)) {
            if (typeof(options.contentWriters[k]) === 'function')
              ...
              try {
                ...
                server._config.contentWriters[k] = require('./contentWriters/'+type[1]);
              } catch (e) {
 ...
 log.trace(JSON.stringify(server._config.contentWriters));
