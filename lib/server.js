...
module.exports = {
  ...
  createServer: function(options) {
    ...
    function httpMain(request, response) {
      ...
      response.events = new EventEmitter();
      ...
      function runChain() {
        return _parseRequest(request, response, function() {
          ...
          if (chain[stage][handler])
            return chain[stage][handler++].call(this, request, response, self);
        });
      }
      ...
      if (route.handlers.pre &&
        ...
        response.events.on('errorSent', function() {
          // Reset to just the post handlers, and then run that chain.  We're
          // assuming that consumers of this manner of API are well-behaved, and
          // aren't going to send an error followed by return next(). That's the
          // whole point to the post chain.
          chain = [route.handlers.post];
          handler = 0;
          stage = 0;
          return runChain();
        });
