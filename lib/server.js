...
function Server(options) {
    ...
    if (options.spdy) {
        // Deprecation warning for SPDY
        console.warn(
            'DEPRECATION WARNING: options.spdy is deprecated and will be ' +
                'removed in a future version. Please use options.http2 ' +
                'instead. SPDY functionality is now handled by native ' +
                'HTTP/2 support.'
        );

        // Map SPDY options to HTTP/2 options for backward compatibility
        options.http2 = options.spdy;
        delete options.spdy;
