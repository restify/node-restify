...
Server.prototype.pre = function pre() {
    ...
    argumentsToChain(handlers).forEach(function forEach(handler) {
        // GH-1639 Bind the server to the this context for the handler, keeping
        // the name as it is (bind changes it to 'bound [name]')
        var boundHandler = handler.bind(self);
        Object.defineProperty(boundHandler, 'name', { value: handler.name });
        self.preChain.add(boundHandler);
    });
...
Server.prototype.use = function use() {
    ...
    argumentsToChain(handlers).forEach(function forEach(handler) {
        ...
        var boundHandler = handler.bind(self);
        Object.defineProperty(boundHandler, 'name', { value: handler.name });
        self.useChain.add(boundHandler);
    });
...
function serverMethodFactory(method) {
    return function serverMethod(opts) {
        ...
        for (var i = 0; i < chain.length; i++) {
            ...
            // GH-1639 Bind the server to the this context for the handler,
            // keeping the name as it is (bind changes it to 'bound [name]')
            chain[i] = chain[i].bind(self);
            Object.defineProperty(chain[i], 'name', { value: name });
        }
