...
RestClient.prototype.put = function(options, callback) {
  ...
  var args = this._processArguments(204, 'PUT', options, callback);
  ...
  this._request(args.options, function(err, code, headers, obj) {
    ...
    if (code !== opts.expect) {
      return cb(newError({
        httpCode: code,
        message: 'HTTP code ' + code + ' != expected code ' + opts.expect,
        details: obj
      }));
    }
...
RestClient.prototype.post = function(options, callback) {
  var args = this._processArguments(200, 'POST', options, callback);
  var opts = args.options;
  var cb = args.callback;
...
RestClient.prototype.del = function(options, callback) {
  var args = this._processArguments(204, 'DELETE', options, callback);
  var opts = args.options;
  var cb = args.callback;
...
RestClient.prototype.head = function(options, callback) {
  var args = this._processArguments(204, 'HEAD', options, callback);
  var opts = args.options;
  var cb = args.callback;
      ...
      res.on('end', function() {
        ...
        if (res.body.length > 0) {
          try {
            ...
            return callback(newError({ error: e }));
          }
        }
