...
function matchURL(re, req) {
        var i = 0;
        var result = re.exec(req.path || req.url);
        var params = {};
...
Router.prototype.mount = function mount(options) {
        ...
        if (routes.some(function (r) { return (r.name === name); }))
                return (false);

        route = {
                name: name,
                method: options.method,
                path: compileURL(options.path),
                version: version
        };
        routes.push(route);
...
Router.prototype.unmount = function unmount(route) {
        assertObject(route);
        assertString(route.method);
        assertObject(route.path);

        var reverse = this.reverse[route.path];
...
Router.prototype.find = function find(req, res, callback) {
        ...
        for (var i = 0; i < routes.length; i++) {
                if (!(params = matchURL(routes[i].path, req)))
                        continue;

                if (routes[i].version.length === 0 || req.version === '*') {
                        r = routes[i];
                        break;
                }

                for (var j = 0; i < r.version.length; j++) {
                        var v = r.version[j];
                        if ((ver = semver.maxSatisfying(v, req.version))) {
                                r = routes[i];
                                break;
                        }
                }
        ...
        if (params && r) {
                ...
                // res.methods = this.reverse[r.path.source].slice();
        }

        if (ver === null)
                return (callback(new InvalidVersionError(req.version)));

        // Check for 405 instead of 404
        var urls = Object.keys(this.reverse);
        for (i = 0; i < urls.length; i++) {
                if (matchURL(new RegExp(urls[i]), req)) {
                        res.setHeader('Allow', this.reverse[urls[i]].slice());
                        var e = new MethodNotAllowedError(req.method +
                                                          ' is not allowed');
                        return (callback(e));
                }
        }
