...
Router.prototype.mount = function mount(options) {
    ...
    if (!this.reverse[host][route.path.source])
        this.reverse[host][route.path.source] = [];

    if (this.reverse[host][route.path.source].indexOf(route.method) === -1)
        this.reverse[host][route.path.source].push(route.method);
...
Router.prototype.unmount = function unmount(name) {
    ...
    var host = route.spec.host || '*';
    var reverse = this.reverse[host][route.path.source];
    var routes = this.routes[route.method];
    this.routes[route.method] = routes.filter(function (r) {
        return (r.name !== route.name);
    });

    this.reverse[host][route.path.source] = reverse.filter(function (r) {
        return (r !== route.method);
    });

    if (this.reverse[host][route.path.source].length === 0)
        delete this.reverse[host][route.path.source];
...
Router.prototype.find = function find(req, res, callback) {
    ...
    // Filter out the routes that do not match the "Host" header
    // of the incoming request. The assumption here is that the
    // routes will be tagged with the "host" attribute.
    routes = routes.filter(function(r) {
        if (!r.spec.host) {
            return 1;
        }
        else {
            var hostMatch = (r.spec.host === req.headers['host']);
            if (hostMatch) {
                cacheKey = req.method + req.absoluteUri(req.url) + req.version() + ct;
            }
            return hostMatch;
        }
    });
    ...
    for (var i = 0; i < routes.length; i++) {
        ...
        if (this.reverse[host]) {
            if (this.reverse[host][routes[i].path.source]) {
                reverse = this.reverse[host][routes[i].path.source];
            }
        }
    ...
    for (i = 0; i < urls.length; i++) {
        if (matchURL(new RegExp(urls[i]), req)) {
            if (this.reverse[host] && this.reverse[host][urls[i]]) {
                res.methods = this.reverse[host][urls[i]].slice();
            }
            else {
