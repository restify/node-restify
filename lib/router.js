...
var fitnessAndQualityParsed = mimeparse.fitnessAndQualityParsed;
var parseMediaRange = mimeparse.parseMediaRange;
...
Router.prototype.find = function find(req, res, callback) {
        ...
        for (var i = 0; i < routes.length; i++) {
                ...
                // On uploads, check if the supplied content-type matches the
                // type of the route; note that we use mimeparse for most
                if (routes[i].type.length !== 0) {
                        ...
                        case 'PUT':
                                tmp = routes[i].type;
                                typed = fitnessAndQualityParsed(ct, tmp);
                                break;
                        ...
                        // mimeparse won't disambiguate
                        // text/plain; foo=bar
                        // from
                        // text/plain; foo=baz
                        // So we do that by hand here by checking the params of
                        // each content-type
                        tmp = tmp[tmp.length - 1];
                        if (!typed ||
                            typed[0] === -1 ||
                            !deepEqual(parseMediaRange(ct), tmp)) {
                                typed = false;
                                continue;
                        }
                }
        ...
        if (!r) {
                ...
                if (ver && ver.key) {
                        r = candidates[ver.key.toString()].route;
                        params = candidates[ver.key.toString()].params;
                } else {
