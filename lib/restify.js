...
function _parseRequest(request, response, next) {
  ...
  if (request.headers[Constants.XApiVersion] ||
      request._config.requireApiVersion) {
    ...
    if (request.headers[Constants.XApiVersion.toLowerCase()] !==
      ...
      return response.sendError(newError({
        ...
        restCode: RestCodes.InvalidArgument,
        message: Constants.XApiVersion + ' must be ' + request._config.apiVersion
      }));
    }
  }
...
module.exports = {
  ...
  createServer: function(options) {
    ...
    server._config.requireApiVersion = false;
    if (options) {
      ...
      if (options.requireApiVersion) {
        server._config.requireApiVersion = true;
      }
    ...
    if (!server._config.apiVersion) {
      server._config.apiVersion = Constants.DefaultApiVersion;
    }
