...
function HttpClient(options) {
  ...
  this.log = this.Logger.child({component: this.name}, true);
...
HttpClient.prototype.request = function request(options, callback) {
  ...
  var log = this.log.child({req_id: options.headers['x-request-id']}, true);
  return operation.attempt(function retryCallback(currentAttempt) {
    ...
    if (log.trace()) {
      ...
      log.trace({req: str, attempt: currentAttempt}, 'Issuing request');
    }

    var req = proto.request(options, function requestCallback(res) {
      res.log =
        self.log.child({req_id: options.headers['x-api-request-id']}, true);
      if (log.trace()) {
        ...
        log.trace({res: resStr}, 'Response received');
      }
    ...
    req.once('error', function onError(err) {
      log.trace({err: err}, 'Request (%s) failed: %s', logId);
