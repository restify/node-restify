...
/**
 * Returns only the media range portion of an accept header or content-type header:
 * 
 * returns 'application/json' from 'application/json; q=1; extension=value'
 * 
 * @param acceptHeader or content-type header
 * 
 * @returns {*}
 */
function normalizeAccept( acceptHeader ){
	if( !acceptHeader ) return null;
	if( acceptHeader.indexOf(';') !== -1 ) acceptHeader = acceptHeader.split(';')[0];
	return acceptHeader ? acceptHeader.toLowerCase() : acceptHeader;
}
...
module.exports = {
    ...
    mergeQs: mergeQs,
	normalizeAccept: normalizeAccept
};
