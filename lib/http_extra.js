...
http.ServerResponse.prototype.send = function(options) {
  ...
  if (_opts.body && _opts.code !== HttpCodes.NoContent) {
    if (this._config.contentWriters[this._accept]) {
      this._config.contentWriters[this._accept](_opts.body, this.request ,this);
    } else {
  ...
  this.headers.Date = this.headers.Date ? this.headers.Date : utils.newHttpDate(now);
  this.headers.Server = this.headers.Server ? this.headers.Server : this._config.serverName;
  for (var k in this._config.headers) {
    if (this._config.headers.hasOwnProperty(k)) {
      var val = this._config.headers[k](this);
      if (val !== undefined && val !== null)
        this.headers[k] = this.headers[k] ? this.headers[k] : val; //allow all of these to be overridden by the writer
    }
  }
  this.__headers = this.headers;

  log.trace('response.send: code=%d, headers=%o, body=%s',
            this.options.code, headers, data);
