...
http.ServerResponse.prototype.send = function(options) {
  ...
  if (_opts.body && _opts.code !== HttpCodes.NoContent) {
    if (this._accept === 'application/x-www-form-urlencoded') {
      ...
      data = this._config.contentWriters[this._accept].call(this, _opts.body);
    } else {
  ...
  if (!this._sent) {
    this.writeHead(this._code, headers);
    if (this._code !== HttpCodes.NoContent && this._method !== 'HEAD' && data)
      this.write(data);
