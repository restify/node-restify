...
function patch(Request) {
    ...
    Request.prototype.absoluteUri = function absoluteUri(path) {
        ...
        // reuse getUrl()'s cached URL instance instead of re-parsing
        // "protocol + hostname + this.path()" into a new URL on every call.
        var base = this.getUrl();
        var firstChar = path.charAt(0);

        if (firstChar === '/') {
            // absolute-path reference: replaces the whole path, origin
            // (protocol/host) stays the current request's.
            return base.origin + path;
        }

        if (firstChar === '?' || firstChar === '#' || path === '') {
            // query-only / fragment-only reference: keep the current
            // (slash-terminated) path, append path as-is.
            return base.origin + base.pathname + '/' + path;
        }

        if (path.indexOf('..') === -1) {
            // plain relative segment (e.g. "child") with no dot-segments to
            // resolve: safe to join directly without invoking `new URL()`.
            return base.origin + base.pathname + '/' + path;
        }

        // relative reference containing ".."/"." segments - fall back to
        // `new URL()` since correctly resolving those requires the real
        // relative-URL-resolution algorithm.
        return new URL(path, base.origin + base.pathname + '/').href;
    };
