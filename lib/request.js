...
function negotiator(req) {
	if( req._negotatiator ) return req._negotiator;
	
    var h = req.headers;
	
	// Normalizing the accept header into the media range only so that the server would 
	// handle an accept a header of 'application/vnd.restify+json;extension=value' when 
	// it is configured such that 'application/vnd.restify' is acceptable and resolves to
	// a valid formatter
	req._negotiator = new Negotatior({
		headers: {
			accept: utils.normalizeAccept( h.accept ) || '*/*',
