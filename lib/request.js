...
function Request(options) {
  ...
  var ka;
  this.__defineGetter__('keepAlive', function () {
    if (ka === undefined) {
      if (self.req.headers.connection) {
        ka = /keep-alive/i.test(self.req.headers.connection);
      } else {
        ka = self.req.httpVersion === '1.0' ? false : true;
      }
    }
    return ka;
  });
